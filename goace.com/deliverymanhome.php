...
<script>
 ...
 <section id="boxes">
            <div class="container">
      <h3>Search Customer's Postcode or Courier</h3>
      <hr>
 <td width="741" align="right">
        <center>
        <form action="deliverymanhome.php" method="get" ecntype="multipart/data-form">
        <input type="text" name="query" style="border:1px solid #CCC; color: #333; width:280px; height:35px;" 
		placeholder="Eg: 02600 / Eg: POSLAJU" /><input type="submit" id="btnsearch" value="Search" name="Search" style="border:5px solid #CCC; color: #333"; />
        </form>
        </center>
        </td>
		<?php 
   ...
   if(isset($_GET['Search'])){
	...
	$sql = "SELECT track.track_id, track.pick_id, pick_status.pick_desc, deliver_status.deliver_desc, 
	track.deliver_id, customer.cust_add, track.track_id, postcode.ps_code, 
	city.city_code, courier.courier_name FROM booking, customer,track, postcode, city, pick_status, deliver_status, 
	courier WHERE track.book_id=booking.book_id AND track.cust_id=customer.cust_id AND booking.courier_id=courier.courier_id AND 
	customer.ps_id=postcode.ps_id AND postcode.city_id=city.city_id AND track.pick_id=pick_status.pick_id AND 
	track.deliver_id=deliver_status.deliver_id AND track.book_id='$bookId' AND
	(postcode.ps_code like '%".$query."%' OR courier.courier_name like '%".$query."%')  ";
   
	$result=mysqli_query($conn, $sql)or die('SQL 4 error');
...
$bil=1;
	...
	<?php
	 while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
		...
		<td><div align="center"><?php echo $row['track_id']; ?></TD>
		<td><div align="center"><?php echo $row['cust_add']; ?></TD>
		<TD><center><?php echo $row['city_code']; ?></TD>
		<TD><center><?php echo $row['ps_code']; ?></TD>
		<td><center><?php echo $row['courier_name']; ?></td>
		<?php if (($row['pick_id'])=='0'){  ?>
		...
		<?php 
				$track_id= $row['track_id'];}  else { ?>   
				<td><center><input type="submit" style="border:1px solid #900; background:#900; height:40px;
				opacity:0.7; width:120px; border-radius:3px; color:#FFF;" 
				value="<?php echo $row['pick_desc']; ?>" disabled></center></td>
		<?php }  ?>
		<?php if (($row['deliver_id'])=='0'){  ?>
		       <form action="pickUpDelivery.php" method="post">
			   <input type="hidden" name="track_id" value="<?php echo $row['track_id']; ?>"> 
		       <TD><center><input type="submit" style="border:1px solid #569c29; background:#569c29; height:40px; 
			   width:105px; border-radius:3px; color:#FFF;" 
		...
		<?php 
				$track_id= $row['track_id'];}  else { ?>   
				<td><center><input type="submit" style="border:1px solid #900; background:#900; height:40px; 
				width:120px; opacity:0.7; border-radius:3px; color:#FFF;" 
				value="<?php echo $row['deliver_desc']; ?>" disabled></center></td>
		<?php }  ?>
		
		<td><center><a href="pickupDelete.php?track_id=<?php echo $row['track_id'];?>">Remove</a></center></td>
		</TR>
	<?php $bil++ ?>
	<?php
		 }
