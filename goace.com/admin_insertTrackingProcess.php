...
<?php
    ...
    if(isset($_POST['Insert']))
    {
       ...
       $trackNo =addslashes($_POST['trackNo']);

       $sql2="UPDATE track set  track_num = '$trackNo' WHERE track_id = $trackId";
       
        if($conn->query($sql2)== TRUE){?>

           <script type="text/javascript">
            alert("Succesfully add tracking number");
            window.location.href = "admin_insertTracking.php";
            </script>  <?php         
        }else{?>

       <script type="text/javascript">
            alert("Oops cannot add tracking number");
            window.location.href = "admin_insertTracking.php";
            </script><?php
    }
...
<br>
    <form action="admin_insertTrackingProcess.php?cust_id=<?php echo $custId;?>" method="POST">
    ...
    <td><input type="text" id="txtbox" name="trackNo" value="" required></td>
    </tr>
